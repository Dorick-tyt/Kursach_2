...
class APIAdapter:
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                if attempt == self.retry_count - 1:
                    raise Exception(
                        f"Ошибка после {self.retry_count} попыток: {str(e)}"
                    )
    ...
    def get_country_coordinates(self, country: str) -> List[str]:
        ...
        if self.test_mode:
            print(
                f"(Тестовый режим) Координаты для {country}: используются тестовые данные"
            )
    ...
    def get_aeroplanes_in_area(self, bounds: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            response = self._make_request_with_retry(
                url=self.opensky_url, params=params
            )
            data = response.json()
    ...
    def get_aeroplanes(self, country: str) -> List[Dict[str, Any]]:
        ...
        print(
            f"   Координаты: S:{coordinates[0]}, N:{coordinates[1]}, W:{coordinates[2]}, E:{coordinates[3]}"
        )
    ...
    def get_top_by_altitude(self, n: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_aeroplanes = sorted(
            self.aeroplanes, key=lambda x: x.get("geo_altitude", 0) or 0, reverse=True
        )
    ...
    def get_top_by_speed(self, n: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_aeroplanes = sorted(
            self.aeroplanes, key=lambda x: x.get("velocity", 0) or 0, reverse=True
        )
    ...
    def filter_by_country(self, country: str) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            if a.get("origin_country", "").lower() == country.lower()
        ]
    ...
    def filter_by_altitude_range(
        ...
        return [
            a
            for a in self.aeroplanes
            if a.get("geo_altitude") is not None
            and min_alt <= a.get("geo_altitude", 0) <= max_alt
        ]

    def filter_by_speed_range(
        self, min_speed: float, max_speed: float
    ) -> List[Dict[str, Any]]:
        """
        ...
        return [
            ...
            and min_speed <= a.get("velocity", 0) <= max_speed
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        velocities = [
            a.get("velocity", 0) for a in self.aeroplanes if a.get("velocity")
        ]
        altitudes = [
            a.get("geo_altitude", 0) for a in self.aeroplanes if a.get("geo_altitude")
        ]

        countries = list(
            set(a.get("origin_country", "Unknown") for a in self.aeroplanes)
        )
    ...
    def print_aeroplanes(
        self, aeroplanes: Optional[List[Dict[str, Any]]] = None, title: str = ""
    ) -> None:
        """
        ...
        for a in aeroplanes:
            ...
            velocity_str = (
                f"{velocity:.1f}" if isinstance(velocity, (int, float)) else "N/A"
            )
            altitude = a.get("geo_altitude")
            altitude_str = (
                f"{altitude:.0f}" if isinstance(altitude, (int, float)) else "N/A"
            )
            on_ground = "Да" if a.get("on_ground") else "✈️ Нет"

            print(
                f"{callsign:<12} {country:<25} {velocity_str:<10} {altitude_str:<12} {on_ground:<10}"
            )
